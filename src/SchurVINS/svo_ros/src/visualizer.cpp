...
{
  // Init traj saver
  const auto logs_dir = boost::filesystem::path("/home/huanhuan/SchurVINS-ws/src/SchurVINS/logs");
  std::cout << "[DEBUG] Logs directory: " << logs_dir << std::endl;
  std::cout << "[DEBUG] Does logs_dir exist?" << boost::filesystem::exists(logs_dir) << std::endl;
