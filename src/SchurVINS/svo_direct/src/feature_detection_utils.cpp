    ...
    const CameraPtr& cam)
{
  ...
  case DetectorType::kSobel:
    ...
    break;
//xinjiade
  case DetectorType::kSuperPoint:
    detector.reset(new SuperPointDetector(options, cam));
	break;
  default:
...
// ============================================================================
// [MOD]  UPDATED IMPLEMENTATION STARTS HERE  [MOD]
// ----------------------------------------------------------------------------
/**
 * Revised version of fillFeatures that is **shrink‑safe**.
 * If the total number of features (existing + new) is **smaller** than the
 * current storage, we create fresh matrices of the exact target size and swap
 * them in — thereby sidestepping Eigen's run‑time error that occurs when
 * attempting to "conservativeResize" to a *smaller* column count.
 */
void fillFeatures(const std::vector<Corner>& corners,
                  const FeatureType&        type,
                  const cv::Mat&            mask,
                  const double&             threshold,
                  const size_t              max_n_features,
                  ...
                  OccupandyGrid2D&          grid)
{
  // ----- copy the candidate features into temporary buffers -----------------
  aslam::Aligned<std::vector, Keypoint>::type keypoint_vec;
  aslam::Aligned<std::vector, GradientVector>::type gradient_vec;
  std::vector<Score> score_vec;
  std::vector<Level> level_vec;
  ...
  const bool need_shrink = (n_tot < static_cast<size_t>(keypoints.cols()));
  if (need_shrink)
  {
    // Allocate fresh storage of the *exact* size we need and copy old data.
    Keypoints new_keypoints  = Keypoints::Zero(2, n_tot);
    Gradients new_gradients  = Gradients::Zero(2, n_tot);
    Scores    new_scores     = Scores::Zero(n_tot);
    Levels    new_levels     = Levels::Zero(n_tot);
    FeatureTypes new_types(n_tot, type);

    const size_t cols_to_copy = std::min(n_old, n_tot);
    if (cols_to_copy > 0)
    {
      new_keypoints.leftCols(cols_to_copy) = keypoints.leftCols(cols_to_copy);
      new_gradients.leftCols(cols_to_copy) = gradients.leftCols(cols_to_copy);
      new_scores.head(cols_to_copy)        = scores.head(cols_to_copy);
      new_levels.head(cols_to_copy)        = levels.head(cols_to_copy);
      std::copy_n(types.begin(), cols_to_copy, new_types.begin());
    }

    keypoints.swap(new_keypoints);
    gradients.swap(new_gradients);
    scores.swap(new_scores);
    levels.swap(new_levels);
    types.swap(new_types);
  }
  else if (n_tot > static_cast<size_t>(keypoints.cols()))
  {
    // Simple expansion (Eigen supports this directly).
    keypoints.conservativeResize(Eigen::NoChange, n_tot);
    gradients.conservativeResize(Eigen::NoChange, n_tot);
    scores.conservativeResize(n_tot);
    levels.conservativeResize(n_tot);
  ...
  // ----- copy the *new* features into the (now correctly‑sized) matrices ----
  for (size_t i = 0; i < n_new; ++i)
  {
    const size_t j = idx[i];
    keypoints.col(n_old + i) = keypoint_vec[j];
...
// ----------------------------------------------------------------------------
// [MOD]  UPDATED IMPLEMENTATION ENDS HERE    [MOD]
// ============================================================================
