    ...
    const FramePtr& cur_frame)
{
  ...
  ROS_INFO_STREAM_THROTTLE(1.0, "[DepthFilter] active seeds = " << ref_frames_with_seeds.size());
    ...
    const float depth_mean)
{
  ...
  ROS_INFO_STREAM("[DepthFilter] max_n_seeds_per_frame: " << max_n_seeds);
  ...
  if(max_n_features <= 0)
  {
    ...
    ROS_INFO_STREAM("[DepthFilter] frame->numFeatures(): " << frame->numFeatures());
    ROS_INFO_STREAM("[DepthFilter] max_n_features for seeds: " << max_n_features);
    return;
  }
  ...
  ROS_INFO_STREAM("[DepthFilter] Seeds initialized this frame: " << n_new);                 
    ...
    const bool use_vogiatzis_update)
{
  ...
  ROS_INFO_STREAM_THROTTLE(1.0, "[updateSeed] trying to update seed[" << seed_index << "]");
  ...
  if(check_visibility)
  {
    ...
    {
      ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: not visible in current image.");
      return false;
    }
    ...
    {
      ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: outside image margin.");
      return false;
    }
  }
  ...
  if(std::isnan(seed::mu(state)))
  {
    ...
    ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: seed mu is NaN.");
    return false;
  }
  ...
  {
    ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: epipolar match failed. Result: " << Matcher::getResultString(res));
    if(!matcher.reject_)
    {
  ...
  if(use_vogiatzis_update)
  {
    if(!updateFilterVogiatzis(
         ...
         state))
    {
      ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: Vogiatzis update failed.");
  ...
  else
  {
    if(!updateFilterGaussian(
         ...
         state))
    {
      ROS_WARN_STREAM_THROTTLE(1.0, "[updateSeed] Seed " << seed_index << " failed: Gaussian update failed.");
      ref_ftr.type = FeatureType::kOutlier;
      return false;
    }
  }
