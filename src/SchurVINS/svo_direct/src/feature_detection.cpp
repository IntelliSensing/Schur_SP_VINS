    ...
    FeatureTypes& types_vec)
{
  ...
  ROS_INFO_STREAM("[Fastdetector] Detected FAST keypoints this frame: " << px_vec.cols());
...
// SuperPointDetector 类实现
void SuperPointDetector::detect(const ImgPyr& img_pyr,
                                const cv::Mat& mask,
                                const size_t max_n_features,
                                Keypoints& px_vec,
                                Scores& score_vec,
...
{
  // 1. 调用 SuperPoint
  static ORB_SLAM2::SPextractor sp_extractor;           // 仅构造一次，避免每帧重复加载
  std::vector<cv::KeyPoint> keypoints;
  cv::Mat dummy;
  sp_extractor(img_pyr[0], mask, keypoints, dummy);

  // 2. 转 Corner 容器
  Corners corners(grid_.n_cols * grid_.n_rows,
                  Corner(0,0,0,0,0));
  for(const auto& kp : keypoints){
    size_t idx = grid_.getCellIndex((int)kp.pt.x, (int)kp.pt.y);
    if(kp.response > corners[idx].score)
      corners[idx] = Corner((int)kp.pt.x, (int)kp.pt.y,
                            kp.response, 0, 0.f);
  }

  // 3. 通过 fillFeatures() 统一写入 px_vec 等
  feature_detection_utils::fillFeatures(
      corners, FeatureType::kCorner, mask, 0.0,
      max_n_features,
      px_vec, score_vec, level_vec, grad_vec, types_vec, grid_);
...
// 注册 SuperPointDetector 工厂方法
/*std::shared_ptr<AbstractDetector> makeSuperPointDetector(
    const DetectorOptions& options,
    const CameraPtr& cam)
{
    return std::make_shared<SuperPointDetector>(options, cam);
}*/
