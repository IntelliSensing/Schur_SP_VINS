        ...
        0)
{
  ...
  if (user_schur_vins) {
    ...
    LOG(INFO) << "SchurVINS Sliding Window Initialized. Window Size: " << WINDOW_SIZE;
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr& frame_bundle)
{
  ...
  ROS_INFO_STREAM("[Keyframes] Total keyframes so far: " << map_->size());
}
...
bool FrameHandlerBase::needNewKf(const Transformation&)
{
  ...
  if (options_.kfselect_min_disparity > 0)
  {
    ...
    if (!disparities.empty())
    {
      ...
      ROS_INFO_STREAM("[Disparity] Frame " << new_frames_->at(0)->id() << " median disparity: " << disparity);
      if (disparity < options_.kfselect_min_disparity)
      {
        // VLOG(40) << "KF Select: NO NEW KEYFRAME disparity not large enough";
        ROS_INFO_STREAM("[KF Select] NO NEW KEYFRAME disparity not large enough");
        return false;
      }
    }
  }
