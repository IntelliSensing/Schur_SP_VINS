...
void SchurVINS::Prediction(double _dt, const Eigen::Vector3d& _acc, const Eigen::Vector3d& _gyr) {
    ...
    // LOG(INFO) << "Calibrated IMU data: acc=[" << acc.transpose() << "], gyr=[" << gyr.transpose() << "]";
    ...
    /* LOG(INFO) << std::fixed << std::setprecision(9)
              << "Exiting Prediction: curr_state->ts=" << curr_state->ts; */
}
...
void SchurVINS::InitState(double _ts, const Eigen::Vector3d& _acc, const Eigen::Vector3d& _gyr) {
    ...
    /* LOG(INFO) << std::fixed << std::setprecision(9)  // 设置日志精度
              << "InitState: " << curr_state->ts << ", quat: " << curr_state->quat.w()
              << ", " << curr_state->quat.x() << ", " << curr_state->quat.y()
              << ", " << curr_state->quat.z(); */
...
void SchurVINS::Forward(const svo::FrameBundle::Ptr frame_bundle) {
    ...
    /* LOG(INFO) << "Number of IMU data in frame_bundle: " << frame_bundle->imu_datas_.size();
    for (const auto& imu_data : frame_bundle->imu_datas_) {
    	LOG(INFO) << "IMU data: timestamp=" << imu_data.timestamp_;
	}  */
    
    double img_ts = frame_bundle->getMinTimestampSeconds();
    
    /* LOG(INFO) << std::fixed << std::setprecision(9)
              << "start forward: img_ts=" << img_ts;   */
    ...
    for (size_t i = 0; i < frame_bundle->imu_datas_.size(); i++) {
        ...
        /* LOG(INFO) << std::fixed << std::setprecision(9)
                  << "IMU data #" << i << ": tmp_ts=" << tmp_ts
                  << ", acc=[" << tmp_acc.transpose() << "]"
                  << ", gyr=[" << tmp_gyr.transpose() << "]";  */
        ...
        if (tmp_ts < curr_state->ts + EPS) {
            ...
            /* LOG(WARNING) << std::fixed << std::setprecision(9)
                         << "Skipping IMU data: tmp_ts=" << tmp_ts 
                         << ", curr_state->ts=" << curr_state->ts; */
            continue;
        }
        ...
        if (tmp_ts < img_ts + EPS) {                                                         
            ...
            /* LOG(INFO) << std::fixed << std::setprecision(9)
                      << "Processing IMU: tmp_ts=" << tmp_ts
                      << ", curr_state->ts=" << curr_state->ts
                      << ", deltaT=" << deltaT;  */
                      
            if (deltaT < 0 || deltaT >= 0.015) {
                LOG(ERROR) << "Invalid deltaT detected: deltaT=" << deltaT;
            }
            ...
            /* LOG(INFO) << std::fixed << std::setprecision(9)
                      << "Interpolating IMU: img_ts=" << img_ts 
                      << ", tmp_ts=" << tmp_ts 
                      << ", curr_state->ts=" << curr_state->ts
                      << ", dt_1=" << dt_1 << ", dt_2=" << dt_2; */
