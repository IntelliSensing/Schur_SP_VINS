...
def run_single_experiment(params, node='svo_ros', node_name='benchmark'):
    ...
    print(f"[DEBUG] Running single experimnet with trace_dir: {params['trace_dir']}")
    ...
    if os.path.exists(dataset_params_file):
        dataset_params = yaml.load(open(dataset_params_file, 'r'), Loader=yaml.FullLoader)
...
def run_experiments(experiment_file, num_monte_carlo_runs=None,
    ...
    experiment_params = yaml.load(open(experiment_params_file, 'r'), Loader=yaml.FullLoader)
    base_params = dict()
    ...
    if 'settings' in experiment_params:
        base_params = dict(list(base_params.items()) +
                           list(experiment_params['settings'].items()))
    ...
    for dataset_cfg in experiment_params['datasets']:
        ...
        if os.path.exists(dataset_params_file):
            dataset_params = yaml.load(open(dataset_params_file, 'r'), Loader=yaml.FullLoader)
            cur_params = dict(list(cur_params.items()) + list(dataset_params.items()))
        else:
        ...
        # 添加的调试信息
        print(f"[DEBUG] Datasetname: {dataset_cfg['name']}")
        print(f"[DEBUG] Trace base directory: {trace_base_dir}")
        print(f"[DEBUG] Dataset trace directory: {dataset_trace_dir}")
        print(f"[DEBUG] Full trace directory: {cur_params['trace_dir']}")                                      
...
if __name__ == "__main__":
    ...
    # 打印输入参数
    print(f"[DEBUG] Experiment file: {args.experiment_file}")
    print(f"[DEBUG] Number of trials: {args.n_trials}")
    print(f"[DEBUG] Alignment type: {args.align_type}")
    print(f"[DEBUG] Number of aligned frames: {args.n_aligned}")
